...
| --- | --- | --- |
| `active` + механизм | 58 | Правило действует и держится гейтом (25), документом (28) или конвейером (5) |
| `active` + `none` | 42 | Правило действует, но здесь ничем не держится — у каждого названа причина |
| `not-applicable` | 79 | Предмета правила в этом проекте нет — с объяснением, почему |

**42 — это метрика, и она должна уменьшаться.** Потолок зафиксирован в
