"""Полнота глоссария относительно Python и инвентарь языка.
...
from glossary import completeness, contracts
...
def test_inventory_ignores_a_foreign_exception_claiming_builtins():
    """`__module__` — заявление, а не факт, и его подделывают не со зла.

    Так делает pytest: его `Skipped`, `Failed` и `Interrupted` объявляют себя
    принадлежащими `builtins`. Они попадали в инвентарь, и снимок зависел от
    того, что ещё импортировал процесс, — контракт менялся от постороннего
    импорта. Сущность обязана находиться в своём модуле, а не ссылаться на него.
    """
    stowaway = type("ЧужоеИсключение", (Exception,), {"__module__": "builtins"})
    try:
        assert "ЧужоеИсключение" not in build_inventory().qualnames
    finally:
        del stowaway


def test_inventory_is_stable_under_foreign_imports():
    """Снимок один и тот же независимо от того, что ещё есть в процессе."""
    before = build_inventory().qualnames
    type("ЕщёОдно", (ValueError,), {"__module__": "builtins"})
    assert build_inventory().qualnames == before
...
def test_missing_card_is_reported():
    report = completeness.build_completeness(make_glossary(), tiny(MEMBER))
...
def test_title_counts_as_a_name():
    ...
    assert (
        completeness.build_completeness(glossary, tiny(EXC))["exceptions"].missing == ()
    )
...
def test_tail_of_a_qualname_does_not_count():
    ...
    assert completeness.build_completeness(glossary, tiny(METHOD))["methods"].missing == (
        "str.split",
    )
...
def test_categories_split_by_kind_and_module():
    report = completeness.build_completeness(
        make_glossary(), tiny(FN, METHOD, EXC, MEMBER)
    )
...
def test_empty_category_counts_as_covered():
    ...
    assert (
        completeness.build_completeness(make_glossary(), tiny(FN))["stdlib"].ratio == 1.0
    )
...
def test_missing_is_sorted():
    ...
    report = completeness.build_completeness(make_glossary(), tiny(MEMBER, other))
...
def test_empty_inventory_is_fully_covered_and_says_so():
    ...
    report = completeness.build_completeness(make_glossary(), tiny())
...
def test_real_glossary_covers_the_builtins(real_glossary: Glossary):
    ...
    assert completeness.build_completeness(real_glossary)["builtins"].ratio == 1.0
...
def test_contract_names_its_schema_and_axis():
    data = completeness.collect(make_glossary(), tiny(FN))
    assert data["schema"] == contracts.SCHEMA
    assert data["schema_of"] == completeness.SCHEMA_OF
...
def test_contract_carries_the_snapshot_digest():
    ...
    data = completeness.collect(make_glossary(make_entry(id="a")), tiny(FN))
...
def test_json_list_is_never_truncated():
    ...
    payload = json.loads(completeness.as_json(make_glossary(), items))
...
def test_json_says_when_it_was_built():
    ...
    payload = json.loads(completeness.as_json(make_glossary(), tiny(FN)))
...
def test_markdown_truncates_and_says_how_to_get_the_rest():
    ...
    text = completeness.as_markdown(make_glossary(), items, limit=5)
...
def test_markdown_names_what_it_cannot_see():
    ...
    text = completeness.as_markdown(make_glossary(), tiny(FN))
