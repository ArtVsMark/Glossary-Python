...
Коды возврата описаны **двумя половинами** — что означают и чего не означают
(правило каталога 056). Вторая половина нужнее первой: именно её отсутствие
превращает «нечего проверять» в «проверено, чисто». Полные описания стоят у
самих констант ниже.
"""
...
EXIT_OK: Final = 0
"""Успех: команда отработала и вывод полон.

Не означает, что данные безупречны: предупреждения валидатора кода возврата не
меняют, и ноль при сотнях предупреждений — обычный день. Не означает и того,
что что-то изменилось: пересборка витрины, уже синхронной со снимком, отвечает
тем же нулём, что и настоящая пересборка.
"""
...
EXIT_USAGE: Final = 2
"""Проверка не отработала: аргументы неверны либо данные не прочитались.

Не означает «нарушений не найдено» — их не искали вовсе. Отличать этот исход
от единицы обязательно: «нечего проверять» и «проверено, чисто» — разные
