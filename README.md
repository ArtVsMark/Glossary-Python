...
| --- | --- | --- |
| `active` + механизм | <!--m:rules_mechanised-->91<!--/m:rules_mechanised--> | Правило действует и держится гейтом (<!--m:rules_gate-->53<!--/m:rules_gate-->), документом (<!--m:rules_document-->28<!--/m:rules_document-->) или конвейером (<!--m:rules_pipeline-->10<!--/m:rules_pipeline-->) |
| `active` + `none` | <!--m:rules_none-->24<!--/m:rules_none--> | Правило действует, но здесь ничем не держится — у каждого названа причина |
| `not-applicable` | <!--m:rules_na-->66<!--/m:rules_na--> | Предмета правила в этом проекте нет — с объяснением, почему |

**<!--m:rules_none-->24<!--/m:rules_none--> — это метрика, и она должна уменьшаться.** Потолок зафиксирован в
...
Оба файла с данными производные: `data/glossary.json` — снимок источника,
`python_glossary.html` — собранная из него витрина. Содержание карточек
правится в источнике и приезжает сюда импортом — см. «Откуда берутся данные»:
...
Правка `data/glossary.json` на месте теряется при следующей выгрузке, правка
`python_glossary.html` — при следующей сборке; CI проверяет синхронность витрины
со снимком и расхождения не пропускает. Замечание к содержанию едет в источник
отчётом `make objections`. Подробности — в [CONTRIBUTING.md](CONTRIBUTING.md),
